...
Podemos montar uma tabela de casos favoráveis para a cada 
...
\begin{table}[h]
  \centering
    \begin{tabular}{ccc}
      \hline
      $X$ & Casos favoráveis & Casos favoráveis (relativo) \\
      \hline
      0 & 1 & 1/4 \\
      1 & 2 & 2/4\\
      2 & 1 & 1/4\\
...
correspondente, como resultado do número relativo de casos favoráveis 
\begin{align*}
P[X=0] &= 1/4 \\
P[X=1] &= 2/4 = 1/2 \\
P[X=2] &= 1/4
...
```{r, results='asis'}
p <- c(.1,.1,.3,.3,.1,.1)
dtab <- matrix(p, 1, dimnames=list('p_i', 0:5))
print(xtable(dtab, digits = rep(1, 7)),
      comment = FALSE,
      sanitize.rownames.function = function(x){
    paste0("$", x, "$")
})
...
esp <- prob * 100
data.frame("Obs" = obs, "p" = round(prob, 2), "Esp" = round(esp, 2), row.names=x)
```
...
x <- 0:7
Obs <- c(9, 38, 71, 115, 125, 106, 79, 50, 57)
prob <- c(dpois(x, lambda = 4.5), 1-ppois(7, lambda = 4.5))
Esp <- round(prob * 650, 2)
data.frame(Obs, "p" = round(prob, 2), Esp, row.names=c(x, '8+'))
```
